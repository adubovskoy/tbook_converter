// Package cli is the convert command's implementation: EPUB/FB2 → .tbook,
// translating every sentence with word-level alignment. cmd/convert is a thin
// wrapper around it; the GUI binary re-execs itself through it in CLI mode.
// Run is single-shot per process (package-level sinks) — never call it twice.
package cli
...
// Main wraps Run with the CLI's exact exit contract: 0 on success and on
// --help (usage already printed by the FlagSet), otherwise `error: <err>` on
// stderr and 1.
...
// Run executes the converter CLI with args = os.Args[1:].
